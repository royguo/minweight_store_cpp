...
`minpatricia` 是 `minweight_store` 使用的有序索引库，也可以作为独立 C++20 library 被外部项目直接链接。它的职责只包括：
...
ByteView                                  `std::span<const std::byte>` 零拷贝 key view
...
性能路径使用 C++20 concepts 和模板静态分发，不通过 virtual interface。自定义 `RecordStore` 需要提供：
...
当前 C++ 实现选择 C++20 作为标准，主要性能策略：

- `Index<RecordStore, NodeStore>` 使用 concepts/templates 静态分发，默认热路径没有 virtual dispatch。
...
已归档的 2026-06-05 benchmark 结果显示，C++ 版本在 1K/10K/100K 上的核心指标均快于 Go 对照，node footprint 与 Go 对齐：
